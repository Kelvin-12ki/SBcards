...
const InsightsIcon = () => <Sparkles className="h-5 w-5" />;
...
const navLinks = [
  { to: '/dashboard', label: 'Dashboard', icon: DashboardIcon, badge: undefined },
  { to: '/messages', label: 'Messages', icon: MessagesIcon, badge: 'unread' as const },
  ...
  { to: '/events', label: 'Matches', icon: MatchesIcon, badge: undefined },
    ...
    const fetchCounts = async () => {
      try {
        const [unread, requests] = await Promise.all([
          ...
          getIncomingRequestsCount().catch(() => 0),
        ]);
        if (!cancelled) {
